...
```{r}
nombres_actuales <- colnames(datos_colegios) # Obtiene los nombres actuales
nuevos_nombres <- str_to_title(nombres_actuales) # 

colnames(datos_colegios) <- nuevos_nombres
```
...
```{r}
datos_colegios_2 <- datos_colegios |> 
...
write.csv(datos_colegios_limpio, file = "datos/colegios_en_csv.csv")
```
...
```{r}
write_xlsx(datos_colegios_limpio, path = "datos/colegios_en_excel.xlsx")
```
...
```{r}
datos_sheets <- list("Información de colegio" = datos_colegios_limpio) 
...
```{r}
datos_sheets <- list("Información colegio limpia" = datos_colegios_limpio,
                      "Información colegio sin procesar" = datos_colegios)
